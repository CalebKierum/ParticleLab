                          ...
                          uint2 gid [[ thread_position_in_grid ]])
{
    ...
    float4 mask_color = mask.read(gid) * 10.0;
                                   ...
                                   uint id [[thread_position_in_grid]])
{
    ...
    float kernelX = float(id);
    float2 respawnAt =  float2(((fast::sin(kernelX * 55.7) + 1.0) / 2.0) * imageWidth, ((cos(kernelX * 33.9) + 1.0) / 2.0) * imageHeight);
    ...
    respawnAt = float2(imageWidth / 2, imageHeight / 2);/* float2((gravityWellOnePosition.x + gravityWellTwoPosition.x + gravityWellThreePosition.x + gravityWellZeroPosition.x) / 4, (gravityWellOnePosition.y + gravityWellTwoPosition.y + gravityWellThreePosition.y + gravityWellZeroPosition.y) / 4);*/
    ...
    const uint2 particlePositionA(ceil(inParticle[0].x), ceil(inParticle[0].y));
    ...
    else if (respawnOutOfBoundsParticles)
    {
        ...
        inParticle[0].x = respawnAt.x;
        inParticle[0].y = respawnAt.y;
    }
    
    const float2 particlePositionAFloat(inParticle[0].x, inParticle[0].y);
    
    const float distanceZeroA = fast::max(distance_squared(particlePositionAFloat, gravityWellZeroPosition), 0.02);
    const float distanceOneA = fast::max(distance_squared(particlePositionAFloat, gravityWellOnePosition), 0.02);
    const float distanceTwoA = fast::max(distance_squared(particlePositionAFloat, gravityWellTwoPosition), 0.02);
    const float distanceThreeA = fast::max(distance_squared(particlePositionAFloat, gravityWellThreePosition), 0.02);
    ...
    const uint2 particlePositionB(ceil(inParticle[1].x), ceil(inParticle[1].y));
    ...
    else if (respawnOutOfBoundsParticles)
    {
        ...
        inParticle[1].x = respawnAt.x;
        inParticle[1].y = respawnAt.y;
    }
    ...
    const uint2 particlePositionC(ceil(inParticle[2].x), ceil(inParticle[2].y));
    ...
    else if (respawnOutOfBoundsParticles)
    {
        ...
        inParticle[2].x = respawnAt.x;
        inParticle[2].y = respawnAt.y;
    }
    
    const float2 particlePositionCFloat(inParticle[2].x, inParticle[2].y);
    
    const float distanceZeroC = fast::max(distance_squared(particlePositionCFloat, gravityWellZeroPosition), 0.02);
    const float distanceOneC = fast::max(distance_squared(particlePositionCFloat, gravityWellOnePosition), 0.02);
    const float distanceTwoC = fast::max(distance_squared(particlePositionCFloat, gravityWellTwoPosition), 0.02);
    const float distanceThreeC = fast::max(distance_squared(particlePositionCFloat, gravityWellThreePosition), 0.02);
    ...
    const uint2 particlePositionD(ceil(inParticle[3].x), ceil(inParticle[3].y));
   // const uint2 particlePositionD(inParticle[3].x, inParticle[3].y);
    ...
    else if (respawnOutOfBoundsParticles)
    {
        ...
        inParticle[3].x = respawnAt.x;
        inParticle[3].y = respawnAt.y;
    }
    
    const float2 particlePositionDFloat(inParticle[3].x, inParticle[3].y);
    
    const float distanceZeroD = fast::max(distance_squared(particlePositionDFloat, gravityWellZeroPosition), 0.02);
    const float distanceOneD = fast::max(distance_squared(particlePositionDFloat, gravityWellOnePosition), 0.02);
    const float distanceTwoD = fast::max(distance_squared(particlePositionDFloat, gravityWellTwoPosition), 0.02);
    const float distanceThreeD = fast::max(distance_squared(particlePositionDFloat, gravityWellThreePosition), 0.02);
...
fragment float4 fragment_copy(texture2d<float> texture [[texture(0)]],
                              ColorInOut texCoord [[stage_in]]) {
    
    constexpr sampler colorSampler(mip_filter::linear,
                                   mag_filter::linear,
                                   min_filter::linear);
    return texture.sample(colorSampler, texCoord.texCoord);
}
